 ...
 * The CompilationEngine class facilitates the conversion of Jack-like language
 * tokens into XML code. It manages classes, variables,
 * subroutines, statements, and expressions via methods like compileClass,
 * compileSubRoutine, compileExpression, etc. This class utilizes a
 * JackTokenizer for tokenization, a SymbolTable for symbol management, and a
 * VMWriter for output generation to a virtual machine. Its functions
 * handle complex parsing tasks for statements and expressions within the code,
 * encapsulating a vital part of the compiler's token-to-XML conversion
 * process.
 */
public class CompilationEngine {
    ...
    public void compileClassVarDec() {
        ...
        // if reach a subroutine, go back in the arraylist to accomodate for advance in
        // the next call
        if (jtoken.keyWord().equals("function") || jtoken.keyWord().equals("method")
                || jtoken.keyWord().equals("constructor")) {
    ...
    public void compileSubRoutine() {
        jtoken.advance();
        // once reach the end, return - no more subroutines - base case for the
        // recursive call
        ...
        if (jtoken.tokenType().equals("KEYWORD") && jtoken.keyWord().equals("void")) {
            strType = "void";
            jtoken.advance();
        } else if (jtoken.tokenType().equals("KEYWORD") && (jtoken.keyWord().equals("int")
                || jtoken.keyWord().equals("boolean") || jtoken.keyWord().equals("char"))) {
    ...
    public void compileParameterList() {
        ...
        // if has parameters add these to symbol table (if the last item in the list, no
        // comma after)
        if (bHasParam) {
            symboltable.define(name, type, "argument");
    ...
    // compiles a sequence of statements, not including the enclosing "{}" - do,
    // let, if, while or return
    ...
    // compiles a term - if current token is an identifier, must distinguish between
    // variable, array entry, and subroutine call
    // single look ahead token which may be "{" "(" or "." to distinguish between
    ...
    public void compileTerm() {
        ...
        if (jtoken.tokenType().equals("IDENTIFIER")) {
            ...
            if (jtoken.tokenType().equals("SYMBOL") && jtoken.symbol() == '[') {
                ...
                compileExpression();
                jtoken.advance();
                // add array number to array start, pop into pointer for that, and push into
                // that
                vmWriter.writeArithmetic("add");
                vmWriter.writePop("pointer", 1);
            ...
            // false and null - 0
                    ...
                    && (jtoken.keyWord().equals("null") || jtoken.keyWord().equals("false"))) {
                vmWriter.writePush("constant", 0);
