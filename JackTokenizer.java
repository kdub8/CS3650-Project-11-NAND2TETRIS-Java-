 ...
 * serves as a parser for Jack code, segmenting it into distinct tokens
 * categorized by type. This class initializes a scanner to read Jack code
 * from a file, removes comments, and organizes tokens into an array list. It
 * discerns keywords, symbols, integer and string constants, and
 * identifiers, crucial for Jack language analysis during compilation. This
 * tokenizer provides essential tokenization for subsequent compilation
 * processes by identifying key language elements like keywords, symbols, and
 * libraries.
 */
public class JackTokenizer {
    ...
    public JackTokenizer(File file) {
        ...
        // keep all input in 1 long string - jackcode, to parse part by part in the
        // advance method
        jackcode = "";
    ...
    // gets next token from input and makes it current token, only called if
    // hasMoreTokens() is true, initially no current token
    public void advance() {
        if (hasMoreTokens()) {
            if (!bFirst) {
                pointer++;
            ...
            // assign current token type and corresponding field variable (keyword, symbol,
            // intval, stringval, or identifier)
    ...
    // returns the type of the current token - keyword, symbol, identifier,
    // int_constant, or string_constant
    public String tokenType() {
        return mTokenType;

    }

    // returns the keyword which is the current token, should be called only when
    // tokenType() is keyword
    public String keyWord() {
        return mKeyWord;
    }

    // returns character which is current token, should be called only when
    // tokenType() is symbol
    public char symbol() {
        return mSymbol;
    }

    // returns identifier which is the current token - should be called only when
    // tokenType() is identifier
    public String identifier() {
        return mIdentifier;
    }

    // returns integer value of the current token - should be called only when
    // tokenType() is INT_CONST
    public int intVal() {
        return mIntVal;
    }

    // returns string value of current token without double quotes, should be called
